...
namespace Rapide.Web.Components.Pages.SystemConfiguration
{
    public partial class SupplierList
    {
        ...
        private MudDataGrid<SupplierModel> dataGrid;
        private string searchString;
        private List<SupplierModel> Supplier = new List<SupplierModel>();
        ...
        private async Task<GridData<SupplierModel>> ServerReload(GridState<SupplierModel> state)
        {
            ...
            data = data.Where(element =>
            {
                ...
                if (element.Name.Contains(searchString, StringComparison.OrdinalIgnoreCase))
                    return true;
                if (element.ContactPerson.Contains(searchString, StringComparison.OrdinalIgnoreCase))
                    return true;
            ...
            if (sortDefinition != null)
            {
                ...
                {
                    case nameof(SupplierModel.Id):
                        data = data.OrderByDirection(
                            sortDefinition.Descending ? SortDirection.Descending : SortDirection.Ascending,
                            o => o.Id
                        );
                        break;
                    case nameof(SupplierModel.Name):
                        data = data.OrderByDirection(
                            sortDefinition.Descending ? SortDirection.Descending : SortDirection.Ascending,
                            o => o.Name
                        );
                        break;
                    case nameof(SupplierModel.ContactPerson):
                        data = data.OrderByDirection(
                            sortDefinition.Descending ? SortDirection.Descending : SortDirection.Ascending,
                            o => o.ContactPerson
                        );
                        break;
                    case nameof(SupplierModel.ContactNumber):
                        data = data.OrderByDirection(
                            sortDefinition.Descending ? SortDirection.Descending : SortDirection.Ascending,
                            o => o.ContactNumber
                        );
            ...
            return new GridData<SupplierModel>
            {
        ...
        {
            NavigationManager.NavigateToCustom("/configurations/suppliers/add");
        }

        private async Task OnDeleteClick(SupplierModel uom)
        {
            try
            {
                if (uom != null)
                {
                    ...
                    if (proceed)
                    {
                        ...
                        await SupplierService.DeleteAsync(uom.Id);
                        SnackbarService.Add("Supplier Successfuly Deleted!", Severity.Normal, config => { config.ShowCloseIcon = true; });

                        IsLoading = false;
                        StateHasChanged();

                        NavigationManager.NavigateToCustom("/configurations/suppliers", true);
                    }
                }
            }
            catch (Exception)
