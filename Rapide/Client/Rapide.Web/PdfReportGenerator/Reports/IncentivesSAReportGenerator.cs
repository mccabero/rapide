...
{
    public static class IncentivesSAReportGenerator
    {
        ...
        private static List<UserDTO> serviceAdvisor { get; set; }
            ...
            List<UserDTO> techniciansDto)
        {
            ...
            var rootFileName = $"INCENTIVES-SA-{dateCoverage.ToUpper()}.pdf";
        ...
        private static void ComposeContent(IContainer container)
        {
            ...
            {
                column.Spacing(1);
                column.Item().Text($"INCENTIVES SA REPORT").FontSize(12).Bold().Underline().AlignCenter();
            ...
            container.Column(column => 
            {
                column.Item().Table(table =>
                {
                    table.ColumnsDefinition(columns =>
                    {
                        ...
                        foreach (var t in serviceAdvisor)
                        {
                    ...
                    table.Header(header =>
                    {
                        ...
                        foreach (var t in serviceAdvisor)
                        {
                            header.Cell().Element(CellStyle).PaddingTop(1).PaddingLeft(25).Text(t.FirstName.ToUpper());
                        }
                    ...
                    var invoicePackageWithIncentives = invoice.Where(x => x.PackageList != null && x.PackageList.Any(y => y.Package.IncentiveSA > 0));
                    var invoiceProductWithIncentives = invoice.Where(x => x.ProductList != null && x.ProductList.Any(y => y.IncentiveSA > 0));

                    List<InvoiceDTO> finalList = new List<InvoiceDTO>();
                    finalList.AddRange(invoicePackageWithIncentives);
                    finalList.AddRange(invoiceProductWithIncentives);
                    ...
                    foreach (var i in finalList)
                    {
                        ...
                        foreach (var t in serviceAdvisor)
                        {
                            decimal incentivesAmount = 0;
                            // check products here?
                            if (i.AdvisorUserId == t.Id)
                            {
                                if (i.JobOrder.ProductList != null)
                                {
                                    incentivesAmount += i.JobOrder.ProductList.Sum(x => x.IncentiveSA);
                                }
                            }

                            if (i.AdvisorUserId == t.Id && i.PackageList != null)
                            {
                                incentivesAmount += i.PackageList.Sum(x => x.Package.IncentiveSA);
                                
                                table.Cell().Element(CellStyle).PaddingRight(45)
                                    .Text(incentivesAmount > 0 ? incentivesAmount.ToString("N2") : "-")
                                    .AlignRight();
                            }
                            else
                            {
                        ...
                        // Total Right
                        decimal incentivesTotalRight = 0;

                        // check products here?
                        if (i.JobOrder.ProductList != null)
                        {
                            incentivesTotalRight += i.JobOrder.ProductList.Sum(x => x.IncentiveSA);
                        }

                        if (i.PackageList != null)
                        {
                            incentivesTotalRight += i.PackageList.Sum(x => x.Package.IncentiveSA);
                            table.Cell().Element(CellStyle).PaddingRight(10).Text(incentivesTotalRight.ToString("N2")).AlignRight();
                        }
                        else
                        {
                            table.Cell().Element(CellStyle).PaddingRight(45).Text("-").AlignRight();
                ...
                column.Item().Table(table =>
                {
                    ...
                    table.Footer(footer =>
                    {
                        ...
                        foreach (var t in serviceAdvisor)
                        {
                            ...
                            var invoiceByAdvisor = invoice.Where(x => x.AdvisorUserId == t.Id);

                            foreach (var i in invoiceByAdvisor)
                            {
                                if (i.PackageList != null)
                                {
                                    totalAmount += i.PackageList.Sum(x => x.Package.IncentiveSA);

                                }
                            }
                            grandTotal += totalAmount;
                            table.Cell().Element(CellStyle).PaddingRight(35).Text(totalAmount > 0 ? totalAmount.ToString("N2") : "-").AlignRight();
                        }
