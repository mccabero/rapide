...
{
    public static class IncentivesTechReportGenerator
    {
            ...
            List<UserDTO> techniciansDto)
        {
            ...
            var rootFileName = $"INCENTIVES-TECH-{dateCoverage.ToUpper()}.pdf";
        ...
        private static void ComposeContent(IContainer container)
        {
            ...
            {
                column.Spacing(1);
                column.Item().Text($"INCENTIVES TECH REPORT").FontSize(12).Bold().Underline().AlignCenter();
            ...
            container.Column(column => 
            {
                column.Item().Table(table =>
                {
                    ...
                    table.Header(header =>
                    {
                        ...
                        header.Cell().Element(CellStyle).PaddingTop(1).Text("WORK DONE");

                        // loop technicians here from user list
                        foreach (var t in technicians)
                        {
                            header.Cell().Element(CellStyle).PaddingTop(1).PaddingLeft(45).Text(t.FirstName.ToUpper());
                        }
                    ...
                    {
                        var pkgWithIncentives = i.PackageList.Where(x => x.Package.IncentiveTech > 0);
                        if (pkgWithIncentives.Any())
                        {
                            // Loop invoice here
                            ...
                            table.Cell().Element(CellStyle).Text(string.Join(", ", pkgWithIncentives.Select(x => x.Package.Name)));

                            // total amount / numbers of technicians
                            var technicianCount = i.JobOrder.TechnicianList.Count;
                            var invoiceAmountForCommission = i.PackageList.Sum(x => x.Package.IncentiveTech) / technicianCount;
                            ...
                            table.Cell().Element(CellStyle).PaddingRight(10).Text(i.PackageList.Sum(x => x.Package.IncentiveTech).ToString("N2")).AlignRight();
                        }
                ...
                column.Item().Table(table =>
                {
                    ...
                    table.Footer(footer =>
                    {
                        ...
                        var grandTotal = packageInvoiceWithIncentives.Sum(x => x.PackageList.Sum(x => x.Package.IncentiveTech));
