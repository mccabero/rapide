...
{
    public static class GatePassReportGenerator
    {
        ...
        public static async Task Generate(JobOrderDTO dto, IJSRuntime JSRuntime, CompanyInfoDTO companyInfoDto)
        {
            ...
            var rootFileName = $"GP{data.ReferenceNo}.pdf";
        ...
        private static void ComposeContent(IContainer container)
        {
            ...
            {
                column.Spacing(1);
                column.Item().PaddingTop(30).Text($"GATE PASS FORM").FontSize(12).Bold().Underline().AlignCenter();
        ...
        private static void ComposeBody(IContainer container)
        {
            container.Column(column =>
            {
                ...
                column.Item().PaddingTop(30).PaddingBottom(30).Text(text =>
                {
                    text.Span("This is to acknowledge that the customer mentioned above already settled the full amount payments from his/her job order with Job Order Number ");
                    text.Span(data.ReferenceNo).Bold().Underline();
                    text.Span(" on ");
                    text.Span(DateTime.Now.ToLongDateString()).Bold();
                    text.Span($". {Environment.NewLine} {Environment.NewLine}" +
                        $"This form will be surendered to Rapide employee as gate pass verification before leaving the premises. ");
                });

                // Vehicle Details Center
                column.Item().AlignCenter().PaddingTop(20).PaddingBottom(30).Text(text =>
                {
                    text.Span($"Vehicle Information: {Environment.NewLine}{Environment.NewLine}");
                    text.Span($"Plate No.: {data.Vehicle.PlateNo} {Environment.NewLine}");
                    text.Span($"Vehicle Make: {data.Vehicle.VehicleModel.VehicleMake.Name} {Environment.NewLine}");
                    text.Span($"Vehicle Model: {data.Vehicle.VehicleModel.Name} {Environment.NewLine}");
                });

                column.Item().PaddingTop(30).PaddingBottom(20).Text("Thank you for trusing Rapide Auto Parts Expert.");
            });
        }
